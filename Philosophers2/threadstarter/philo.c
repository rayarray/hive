...
/*   Updated: 2023/08/14 15:57:06 by rleskine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*threadfunc(void *arg)
{
	...
	while (b->i--)
	{
		pthread_mutex_lock(b->slp);
		pthread_mutex_unlock(b->slp);
		usleep(500);
	}
...
int	commence_philosophy(t_table *t)
{
	...
	pthread_create(&t->timer, NULL, sleeper, &t->slp);
	i = 0;
	while (++i <= t->philos)
		make_brain(i, t->b, &t->slp, t->philos);
	i = -1;
	while (++i < t->philos)
	{
...
int	main(int ac, char **ag)
{
	...
	if (pthread_mutex_init(&t.slp, NULL))
	...
	while (++t.i <= t.philos)
	{
		frst = cmp_tv(frst, &((t.b + t.i)->exited), 0);
		last = cmp_tv(last, &((t.b + t.i)->exited), 1);
	}
	printf("earliest thread started at %lds, %dus\n", frst->tv_sec % 100, frst->tv_usec);
	printf("latest thread started at %lds, %dus\n", last->tv_sec % 100, last->tv_usec);
