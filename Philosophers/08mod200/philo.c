...
/*   Updated: 2023/08/10 14:56:29 by rleskine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

// todo single philo must die
...
void	commence_philosophy(int size, pthread_t philos[],
	pthread_mutex_t **forks, t_pdata *brains)
{
	int	result;
	int	i;
	...
	i = 0;
	while (++i <= size)
		make_brain(i, brains, forks, size);
	i = 0;
	while (++i <= size)
		printf("p%dwait%d\n", i, (brains + i)->wait);
	i = 0;
